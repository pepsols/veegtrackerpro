...
class TrackingService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                currentRunId = intent.getLongExtra(EXTRA_RUN_ID, -1)
                startTracking()
            }
    ...
    private fun saveLocation(lat: Double, lon: Double, accuracy: Float?, speed: Float?) {
        if (currentRouteId == -1L || currentRunId == -1L) return
        serviceScope.launch {
